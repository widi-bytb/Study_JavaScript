...
//  可以看到，Promise的写法只是回调函数的改进，使用then方法以后，异步任务的两段执行看的更清楚了。除此以外，并无新意
...
// 上面代码的函数asyncJob是一个协程，它的奥妙就在其中的yield命令。他表示执行到此处，执行权将交给其他协程。也就是说，yield命令式异步两个阶段的分界线。

// 协程遇到yield命令就暂停，等到执行权返回，再从暂停的地方继续往后执行。它的最大优点，就是代码的写法非常像同步操作，如果去除yield命令，简直一摸一样。
...
// 整个Generator函数就是一个封装的异步任务，或者说是异步任务的容器。异步操作需要暂停的地方，都用yield命令语句注明。Generator函数的执行方法如下
{
    function* gen(x) {
        var y = yield x + 2;
        return y;
...
// 上面代码中，第一个next方法的value属性，返回表达式x+2的值3，第二个next方法带有参数2，这个参数可以传入Generator函数，作为上个阶段异步任务的返回结果，被函数体内的变量y接受。因此，这一步的value属性，返回的就是2（变量y的值）

// Generator函数内部还可以部署错误处理代码捕获函数体外抛出的错误。
{
    function* gen(x) {
        try {
            var y = yield x + 2;
    ...
    var g = gen(1);
    // console.log(g.next())
    // console.log(g.throw('出错误了'))
    // { value: 3, done: false }
    // 出错误了
    // { value: undefined, done: true }
}
// 上面代码的最后一行，Generator函数体外，使用指针对象的throw方法抛出的错误，可以被函数体内的try...catch代码块捕获。这意味着，出错的代码与处理错误的代码，实现了时间和空间上的分离。这对异步编程无疑是很重要的
...
// 那时,编程语言刚刚起步,计算学家还在研究,编译器怎么写比较好.一个争论的焦点是"求值策略",即函数的参数到底应该何时求值
{
    var x = 1;
    function f(m) {
        return m * 2;
...
// 另一种意见“传名调用”（call by name），即直接将表达式x+5传入函数体，只在用到它的时候求值。Haskell语言采用这种策略
{
    // f(x+5)
    // 传名调用时，等同于
    // (x+5) * 2
...
// 上面代码中，函数f的第一个参数是一个复杂的表达式，但是函数体内根本没用到。对这个参数求值，实际上是不必要。因此，有一些计算机学家倾向于“传名调用”，即只在执行时求值
...
// 上面代码中，函数f的参数x+5被一个函数替换了。凡使用到原参数的地方，对Thunk函数求值即可

// 这就是Thunk函数的定义，他是”传名调用“的一种是实现策略，用来替换某个表达式。
...
// 上面代码中，只要Generator 函数还没执行到最后一步，next函数就调用自身，以此实现自动执行



// TODOco模块的源码
// co就是上面那个自动执行器的扩展，它的源码只有几十行，非常简单。
// 首先，co函数接受Generator 函数作为参数、返回一个Promise对象
{
    function co(gen) {
        var ctx = this;
...
// 在返回的Promise对象里面，co先检查参数gen是否为Generator 函数。如果是，就执行该函数，得到一个内部指针对象；如果不是就返回，并将Promise对象的状态改为resolved.
{
    function co(gen) {
        var ctx = this;
...
// 接着，co将Generator 函数的内部指针对象的next方法，包装成onFulfilled函数。这主要是为了能够捕捉抛出的错误。
{
    // function co(gen) {
    //     var ctx = this;
...
// 第一行，检查当前是否为Generator 函数的最后一步，如果是就返回。

// 第二行，确保每一步的返回值，是Promise对象

// 第三行，使用then方法，为返回值加上回调函数，然后通过onFulfilled函数再次调用next函数。

// 第四行，在参数不符合要求的情况下（参数非Thunk函数和Promise对象），将Promise对象的状态改为reject,从而终止执行。
