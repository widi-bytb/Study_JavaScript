...
// 前文有一个 Generator 函数，依次读取两个文件。
{
    ...
    const gen = function* () {
        const f1 = yield readFile('D:/readFile/a.txt');
        const f2 = yield readFile('D:/readFile/b.txt');
...
// 上面代码的函数gen可以写成async函数，就是下面这样
{
    ...
    const asyncReadFile = async function () {
        const f1 = await readFile('D:/readFile/a.txt');
        const f2 = await readFile('D:/readFile/b.txt');
        console.log(f1.toString());
        console.log(f2.toString());
    };
    // asyncReadFile();
...
// 上面的代码调用了asyncReadFile函数，然后它就会自动执行，输出最后的结果。这完全不像Generator函数，需要调用next方法，或者用co模块，才能真正执行，得到最后的结果。
...
// 进一步说，async函数完全可以看作多个异步操作，包装成的一个Promise对象，而await命令就是内部then的语法糖、
...
// 这个例子还演示了如何实现休眠的效果。JavaScript一直没有休眠的语法，但是借助await命令就可以让程序停顿指定的时间。下面给出了一个简化的sleep实现
{
...
// await命令后面的Promise 对象如果变为reject状态，则reject的参数会被catch方法的回调函数接收到。
{
    async function f() {
...
// 任何一个await语句后面的Promise对象变为reject状态，那么整个async函数都会中断执行async 
{
    async function f() {
        await Promise.reject('出错了');
...
// 上面代码中，第二个await语句是不会运行的，因为第一个await语句状态变成了reject。
...
// 另一种方法是await后面的Promise 对象再跟一个catch方法，处理前面可能出现的错误
{
    // async function f() {
    //     await Promise.reject('出错了')
    //         .catch(e => console.log(e));
...
// 虽然Promise的写法比回调函数的写法大大该进，但是一眼看上去，代买完全都是Promise的API（then,catch等等），操作本身的语义反而不容易看出来
// 接着是Generator函数的写法
{
...
// 可以看到ASync函数的实现最简洁，最符合语义，几乎没有语义不相关的代码。他将Generator写法中的自动执行器，改在语言层面提供，不暴露给用户，因此代码量最少。如果使用Generator写法，自动执行器需要用户自己提供



// [*]6.实例：按顺序完成异步操作
// 实例开发中，经常遇到一组异步操作，需要按照顺序完成。比如，依次远程读取一组URL，然后按照读取的顺序输出结果
// Promise的写法如下
{
...
// [*]7.顶层await
// 早期的语法规定是，await命令只能出现在async函数内部，否则都会报错。
{
    // 报错
...
// 目前的解决方法，就是让原始模块输出一个Promise 对象，从这个Promise 对象判断异步操作有没有结束
{
    // awaiting.js
    // let output;
...
// 上面代码中，awaiting.js出了输出output，还默认输出一个Promise 对象(async函数立即执行后，返回一个 Promise对象)，从这个对象判断异步操作是否结束
...
// 上面代码中，将awaiting.js对象的输出，放在promise.then()里面，这样就能保证异步操作完成以后，才去去读output
...
// 上面代码的写法，与普通的模块加载完全一样。也就是说，模块的使用者完全不用关心，依赖，模块的内部有没有异步操作，正常加载即可

// 这时，模块的加载会等待依赖模块（上例是awaiting.js）的异步操作完成，才执行后面的代码，有点像暂停在那里。所以，它总是会得到正确的output，不会因为加载时机的不同，而得到不一样的值。

// 注意，顶层await只能用在ES6模块，不能用在CommonJS模块。这是因为CommonJS模块的require()是同步加载，如果有顶层await，就没法处理加载了
...
// 顶层的await命令有点像，交出代码的执行权给其他的模块加载，等异步操作完成，再拿回执行权，继续向下执行。
