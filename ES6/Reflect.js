...
//     现阶段，某些方法同时在Object和Reflect对象上部署，未来的新方法只部署在Reflect对象上，也就是说，从Reflect对象上可以拿到语言内部的方法。
...
// 重点如果name属性部署了读取函数(getter),则读取函数的this绑定receiver
{
    let myObject = {
        foo: 1,
        bar: 2,
...
// 注意，如果Proxy对象和Reflect对象联合使用，前者拦截赋值操作，后者完成度值的默认行为，而且转入了receiver,那么Reflect.set会触发Proxy.defineProperty拦截
{
    let p = {
        a: 'a'
    };
    ...
    // 这时因为Proxy.set的receiver参数总是指向当前的Proxy实例(即上例的obj)，而Reflect.set一旦传入receiver，
    // 就会将属性赋值刀到receiver上面(即obj)，导致触发defineProperty拦截。如果Reflect.set没有传入receiver，那么就不会触发defineProperty拦截
...
// Reflect.has方法对应name in obj 里面的in 运算符。
{
    var myObject = {
        foo: 1,
    };
...
// 如果Reflect.defineProperty()方法的第一个参数不是对象，会报错

// [*] Reflect.construct(target,args),这提供了一种不使用new，来调用构造函数的方法。
{
...
// Reflect.setPrototypeOf方法用于设置目标对象的原型(prototype),对应Object.setPrototypeOf(obj, newProto)方法，他返回一个布尔值，表示是否设置成功
{
    const myObj = {};
...
// [*] Reflect.apply(fuc,thisArg,args)
// Reflect.apply方法等同于Function.prototype.apply.call(fuc,thisArg, args),用于绑定题this对象后执行给定函数

// 一般来说，如果要绑定一个函数的this对象，可以这样写fu.apply(obj, args),但是如果函数定义了自己的apply方法，就只能写成Function.prototype.apply.call(fuc,thisArg, args),采用Reflect对象可以简化这种操作
{
...
// 这个方法可以于Proxy.defineProperty配合使用
{
    const p = new Proxy({}, {
...
// Reflect.getOwnPropertyDescriptor基本等同于Object.getOwnPropertyDescriptor，用于得到指定属性的描述对象，将来会替代掉后者。
{
    ...
    // 新写法
    let theDescriptor1 = Reflect.getOwnPropertyDescriptor(myObject, 'hidden');
}
